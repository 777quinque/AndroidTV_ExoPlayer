...
class TelechannelsHDFHD : AppCompatActivity() {
    ...
    for (line in lines) {
        when {
            line.startsWith("#EXTINF") -> {
                ...
                name = line.substringAfter(",") // Можно использовать другой способ для извлечения названия
                imageUrl = line.substringAfter("tvg-logo=\"").substringBefore("\"")
            }
            line.startsWith("http") -> {
                videoURL = line
                if (groupTitle == "ТЕЛЕКАНАЛЫ HD-FHD") {
